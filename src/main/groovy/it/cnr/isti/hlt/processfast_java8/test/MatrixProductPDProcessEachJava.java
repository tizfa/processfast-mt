...
package it.cnr.isti.hlt.processfast_java8.test;
...
import it.cnr.isti.hlt.processfast_java8.core.GParsRuntime;
                ...
                (TaskContext tc) -> {
                    ...
                    pd1.enableLocalComputation(true).processEach(
                            (TaskDataContext tdc, Pair<Integer, Integer> item) -> {
                                double[][] m1 = tdc.getTaskSharedData().getDataDictionary().get("matrix1");
                                double[][] m2 = tdc.getTaskSharedData().getDataDictionary().get("matrix2");
                                double[][] res = tdc.getTaskSharedData().getDataDictionary().get("result");
                                int i = item.getV2();
